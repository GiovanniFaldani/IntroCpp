...
class EserciziLezione2OperatoriEStruttureDiControllo
{
private:

public:

	EserciziLezione2OperatoriEStruttureDiControllo();
...
class EserciziLezione2Puntatori
{
private:

public:

	EserciziLezione2Puntatori();
	...
	// Esercizio 3
	void raddoppia(int* p_valore);

	// Esercizio 4
	void scambia(int* valore1, int* valore2);
...
"L'area raddoppiata supera i 100." se area � maggiore di 100.
"L'area raddoppiata � 100 o meno." altrimenti.
...
All'interno del ciclo, usa l'operatore modulo (%) e una condizione if per determinare se il numero corrente � pari.
...
Usa un ciclo while che continua finch� contatore � maggiore di 0.
...
Usa un ciclo do-while che si ripete finch� continua � true.
All'interno del ciclo, chiedi all'utente di inserire un carattere ('S' per s�, 'N' per no).
...
Scrivi una singola condizione if che valuti se lo studente � promosso. La regola �:
Deve avere pi� di requisitoMinimo punti E le presenze devono essere uguali o maggiori di 80.
...
Usa un ciclo for-each per stampare ogni elemento della collezione, preceduto dalla stringa "Il colore �: ".
...
Usa una condizione if con l'operatore & per verificare se il permesso di Scrittura � attivo:
if (permessi & MASCHERA_SCRITTURA)
Stampa "Il permesso di scrittura � attivo." o "Il permesso di scrittura NON � attivo."
